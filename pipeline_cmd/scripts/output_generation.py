...
from scripts.analysis import get_c_source
...
def summary_output(scores_output, scores_bb, out_opt=None):
    ...
    rank.to_csv("./outputs/partof_summary_output.csv", index = True, mode='a', header=False)
    
    #read the full csv file as a DataFrame
    df = pd.read_csv("./outputs/partof_summary_output.csv")
    ...
    df.to_csv("./outputs/summary_output.csv", index = False, mode='a', header=True)
...
def metabolic_detailed_output(input_file, consumption, final):
    ...
    with open('./outputs/metabolic_detailed_output.csv', 'w', newline='') as csvfile:
...
def reaction_details(scores_output, final, to_wiki, out_harm):
    ...
    """
    df = pd.read_csv("./outputs/metabolic_detailed_output.csv")
    ...
    df.to_csv("./outputs/detailed_output.csv", index = False, mode='a', header=True)
...
def write_data_provenance(data_repo, input_file):
    ...
    """
    with open('./outputs/data_provenance.txt', 'w') as file:
        #write time and date
        now = datetime.now()
